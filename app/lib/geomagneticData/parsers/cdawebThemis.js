  ...
  return cdaweb(filePath).then((data) => {
    if (data.properties) {
      ...
      if (data.properties.logicalSourceDescription) {
        let coord = data.properties.logicalSourceDescription.match(/\(lat\s+(\d+\.\d+),\s+long\s+(\d+\.\d+)\)/i);
        if (coord !== null) {
    ...
    if (data.columns) {
      let columnKeys = [0, 2, 3, 4];
      data.columns = data.columns.filter((column, i) => columnKeys.indexOf(i) !== -1);
      data.rows = data.rows.map((row) => {
        return row.filter((column, i) => columnKeys.indexOf(i) !== -1);
      })
    }
