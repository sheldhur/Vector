        ...
        if (line !== '') {
          if (isProperty) {
            if (line.startsWith('-----')) {
              ...
              let lineSplit = line.match(regexp.data);
              if (lineSplit.length === 3) {
                lineSplit[2].match(regexp.values).forEach((value) => {
                  let stationName = value.trim().split(/\s+/)[0];
            ...
            let lineSplit = line.match(regexp.data);
            if (lineSplit.length === 3) {
              let date = moment(lineSplit[1].trim(), "YYYY MM DD HH mm ss").toDate();
              let values = lineSplit[2].match(regexp.values);
              data.forEach((station) => {
                let stationValues = values.splice(0, 3).map((item) => parseFloat(item));
