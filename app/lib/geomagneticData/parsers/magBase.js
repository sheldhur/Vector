  ...
  return new Promise((resolve) => {
      fs.readFile(filePath, (error, rawData) => {
        ...
        while (position < rawData.length) {
          let buffer = rawData.slice(position, Math.min(position + bufferSize, rawData.length));
          position += bufferSize;

          let line = binaryParser.parse(buffer);
          ...
          if (line.glon === NO_DATA || line.gnpd === NO_DATA || line.glon === 0 || line.gnpd === 0) {
            ...
            line.glon = line.glon * 0.01;
            line.gnpd = line.gnpd * 0.01;
