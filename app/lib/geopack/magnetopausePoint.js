...
export default class MagnetopausePoint {
  ...
  calculate(latitude, longitude) {
    const {bz, pressureSolar, pressureMagnetic, dipoleTilt} = this;
    ...
    if (pressureMagnetic > 0 && pressureSolar > 0 && bz > -200) {
      ...
      let en = A[21];
      let es = A[21];
      let tn = A[19] + A[20] * dipoleTiltRad;
      let ts = A[19] - A[20] * dipoleTiltRad;

      let xn = Math.acos(Math.cos(te) * Math.cos(tn) + Math.sin(te) * Math.sin(tn) * Math.cos(fi - Math.PI / 2));
      let xs = Math.acos(Math.cos(te) * Math.cos(ts) + Math.sin(te) * Math.sin(ts) * Math.cos(fi - 3 * Math.PI / 2));

      let b0 = A[6] + A[7] * (Math.exp(A[8] * bz) - 1) / (Math.exp(A[9] * bz) + 1);
      let b1 = A[10];
      let b2 = A[11] + A[12] * dipoleTiltRad;
      let b3 = A[13];

      let cn = A[14] * Math.pow(pressureSolar + pressureMagnetic, A[15]);
      let cs = cn;
      let dn = A[16] + A[17] * dipoleTiltRad + A[18] * Math.pow(dipoleTiltRad, 2);
      let ds = A[16] - A[17] * dipoleTiltRad + A[18] * Math.pow(dipoleTiltRad, 2);
      ...
      //Rmp
      let altitude = f0 * f + cn * Math.exp(dn * Math.pow(xn, en)) + cs * Math.exp(ds * Math.pow(xs, es));
